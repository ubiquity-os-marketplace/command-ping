...
function decompressBase64(buffer) {
  ...
  throw new Error(`Unable to decompress input (tried ${attempts.map((a) => a.name).join(', ')}): ${errors.join(' | ')}`);
}
...
async function postIssueComment({ owner, repo, issueNumber, body, token }) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'user-agent': 'ubiquity-os-marketplace/command-ping',
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to create comment: ${response.status} ${response.statusText}${responseText ? `\n${responseText}` : ''}`);
  }
...
async function main() {
  ...
  if (!owner || !repo || !issueNumber) {
    throw new Error('Could not resolve repository owner/name/issue number from event payload');
  }

  await postIssueComment({ owner, repo, issueNumber, body: 'pong', token: authToken });
  console.log(`Replied with pong on ${owner}/${repo}#${issueNumber}`);
}
